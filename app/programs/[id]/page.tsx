...
export default async function ProgramDetailPage(props: any) {
  ...
  function parseHighlights({ highlights }: { highlights: string | null }): ParsedHighlights {
    ...
    const facts = details.reduce<HighlightFact[]>((acc, line) => {
      const match = line.match(/^(?<label>[^:]+):\s*(?<value>.+)$/)
      if (!match?.groups?.value) return acc
  ...
  const { summary: highlightSummary, facts: highlightFacts, bullets: highlightBullets } = highlightContent
  const formattedHighlightSummary =
    highlightSummary && /[.!?]$/.test(highlightSummary) ? highlightSummary : highlightSummary ? `${highlightSummary}.` : null

  const curriculumSections = parseCurriculumSections({ curriculum: program.curriculum, abbreviations: curriculumAbbreviations })
  const hasStructuredCurriculum = curriculumSections.length > 0
  const eligibilityContent = parseEligibility({ eligibility: program.eligibility })
  ...
  const emiAmount = program.emiAvailable && program.totalFee ? Math.round(program.totalFee / 24) : null;

  const applyUrl = program.applyUrl ?? '/lead-form';
  const isExternalUrl = applyUrl.startsWith('http://') || applyUrl.startsWith('https://');

  const ApplyNowLink = ({ className, style }: { className?: string; style?: React.CSSProperties }) => {
    const commonProps = {
    ...
    if (isExternalUrl) {
      return <a href={applyUrl} {...commonProps}>Apply Now</a>;
    }
    return <Link href={applyUrl} {...commonProps}>Apply Now</Link>;
  };
      ...
      <section className="pt-24 pb-12 px-6 bg-gradient-to-b from-slate-950/5 via-white to-slate-50">
        <div className="max-w-7xl mx-auto space-y-8 lg:space-y-10">
          <div className="relative overflow-hidden rounded-[32px] bg-slate-900 shadow-2xl">
            {heroSrc ? (
              <img src={heroSrc} alt={program.title} className="h-[420px] w-full object-cover md:h-[520px]" />
            ) : (
                ...
                <div className="max-w-3xl space-y-6">
                  <div className="flex flex-wrap items-center gap-4">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/15 bg-white/10 backdrop-blur md:h-16 md:w-16">
                      <img src={logoSrc} alt={program.institutionName} className="h-9 w-9 object-contain md:h-10 md:w-10" />
                    </div>
                    <div>
                      <p className="text-xs font-medium uppercase tracking-[0.24em] text-slate-300">Offered by</p>
                      <p className="text-sm font-semibold text-white md:text-base">{program.institutionName}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    <h1 className="text-3xl font-bold leading-tight text-white md:text-5xl md:leading-[1.05]">{program.title}</h1>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,320px)] lg:items-start">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl lg:p-8">
              <h2 className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Program Snapshot</h2>
              <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 text-center">
                  <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500">Mode</div>
                  <div className="mt-2 text-base font-semibold text-slate-900 capitalize md:text-lg">{program.deliveryMode ?? '—'}</div>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 text-center">
                  <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500">Duration</div>
                  <div className="mt-2 text-base font-semibold text-slate-900 md:text-lg">{program.durationMonths ? `${program.durationMonths} months` : '—'}</div>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 text-center">
                  <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500">Total Fee</div>
                  <div className="mt-2 text-base font-semibold md:text-lg" style={{ color: 'var(--brand)' }}>
                    {program.totalFee ? `₹${program.totalFee.toLocaleString('en-IN')}` : 'Contact Us'}
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 text-center">
                  <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500">EMI from</div>
                  <div className="mt-2 text-base font-semibold text-emerald-600 md:text-lg">
                    {emiAmount ? `₹${emiAmount.toLocaleString('en-IN')}/month` : '—'}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                {program.emiAvailable && emiAmount && (
                  <div className="mt-1 rounded-2xl border border-purple-100 bg-purple-50/70 px-4 py-3 text-sm text-purple-900">
                    Flexible EMI plans starting at <span className="font-semibold">₹{emiAmount.toLocaleString('en-IN')}/month</span>. Talk to our counsellor to customise your plan.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 space-y-12">
              ...
              <div className="bg-white rounded-3xl shadow-xl p-8 border border-slate-200">
                ...
                <div className="prose max-w-none text-slate-700 leading-relaxed">
                  {formattedHighlightSummary || highlightFacts.length > 0 || highlightBullets.length > 0 ? (
                    <div className="space-y-6">
                      {formattedHighlightSummary && (
                        <p className="text-lg text-slate-700">{formattedHighlightSummary}</p>
                      )}
                      {highlightFacts.length > 0 && (
                        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                          {highlightFacts.map(fact => (
                            <div key={`${fact.label}-${fact.value}`} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                              <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{fact.label}</dt>
                              <dd className="mt-2 text-lg font-semibold text-slate-900">{fact.value}</dd>
                            </div>
                          ))}
                        </dl>
                      )}
                      {highlightFacts.length === 0 && highlightBullets.length > 0 && (
                        <ul className="space-y-3">
                          {highlightBullets.map((highlight, idx) => (
                            <li key={idx} className="flex items-start gap-3">
                              <span className="mt-2 inline-block h-2 w-2 flex-shrink-0 rounded-full" style={{ backgroundColor: 'var(--brand)' }}></span>
                              <span>{highlight}</span>
                            </li>
                          ))}
                        </ul>
                    ...
                    hasStructuredCurriculum ? (
                      <div className="overflow-hidden rounded-3xl border border-slate-200 shadow-sm">
                        {curriculumSections.map((section, index) => (
                          <div
                            key={section.title}
                            className={`grid gap-6 p-6 md:grid-cols-[220px_1fr] ${index !== curriculumSections.length - 1 ? 'border-b border-slate-100' : ''}`}
                          >
                          ...
                          {section.items.map((item, itemIdx) => (
                            <li key={`${section.title}-${itemIdx}`} className="flex items-start gap-3">
                                  <span
                  ...
                  {program.eligibility ? (
                    <div className="space-y-4">
                      ...
                      {hasEligibilityRequirements && (
                        <ul className="space-y-2 text-slate-700">
                          {eligibilityContent.requirements.map((item, requirementIdx) => (
                            <li key={`eligibility-${requirementIdx}`} className="flex items-start gap-3">
                              <span
                                className="mt-[6px] inline-block h-2 w-2 flex-shrink-0 rounded-full"
                                style={{ backgroundColor: 'var(--brand)' }}
                              ></span>
                              <span className="leading-relaxed">{ensureTrailingPeriod(item)}</span>
                            </li>
                          ))}
                        </ul>
                      )}
                      {!eligibilityContent.summary && !hasEligibilityRequirements && (
            ...
            <div className="lg:self-start">
              ...
              <div className="bg-white rounded-3xl shadow-xl p-6 md:p-8 border border-slate-200 lg:sticky lg:top-24 lg:max-w-md lg:w-full">
                ...
                <div className="space-y-8">
                  {hasProgramLogistics && (
                    <section>
                      <h4 className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">Program Logistics</h4>
                  ...
                  {hasFinancialSummary && (
                    <section>
                      <h4 className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">Investment Snapshot</h4>
                      <dl className="mt-4 grid gap-3">
                        {financialSummary.map(item => (
                          <div key={item.label} className="flex items-baseline justify-between gap-3 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
                  ...
                  {hasFeeBreakdown && (
                    <section>
                      <h4 className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">Fee Breakdown</h4>
                            ...
                            <tbody className="divide-y divide-slate-100 bg-white">
                              {feeBreakdownRows.map(row => (
                                <tr key={row.id}>
                                  ...
                                  <td className="px-4 py-3 text-right font-semibold text-slate-900">{row.amount}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
...
function parseCurriculumSections({ curriculum, abbreviations }: { curriculum: string | null, abbreviations: CurriculumAbbreviation[] }): CurriculumSection[] {
  ...
  const sections = normalized
    ...
    .split(/(?=(?:Sem(?:ester)?|Trimester|Term|Module|Year)\s*\d+(?:[\s:–—-]|$))/gi)
    .map(segment => segment.trim())
    .filter(segment => segment.length > 0)
...
function expandCurriculumAbbreviation({ topic, abbreviations }: { topic: string, abbreviations: CurriculumAbbreviation[] }): string {
  const found = abbreviations.find(entry => entry.key.toLowerCase() === topic.toLowerCase())
  if (found) return found.value
  return topic
}

function extractCurriculumSection({ segment, abbreviations }: { segment: string, abbreviations: CurriculumAbbreviation[] }): CurriculumSection | null {
  const headingRegex = /^(?<core>(?:Sem(?:ester)?|Trimester|Term|Module|Year)\s*\d+)(?:\s*[–—-]\s*(?<extra>[^:•\n]+))?(?:[:\-–—]\s*)?(?<body>.+)$/i
  ...
  if (headingMatch?.groups) {
    coreHeading = headingMatch.groups.core?.trim() ?? null
    extraHeading = headingMatch.groups.extra?.trim() ?? null
    remainder = headingMatch.groups.body?.trim() ?? ''
  } else {
    const [firstLine, ...rest] = segment.split(/\r?\n/).map(line => line.trim()).filter(Boolean)
    if (firstLine && /^(?:Sem(?:ester)?|Trimester|Term|Module|Year)\s*\d+/i.test(firstLine)) {
  ...
  const topics = rawTopics
    .filter(topic => !/^(?:Sem(?:ester)?|Trimester|Term|Module|Year)\s*\d+/i.test(topic))
...
function formatCurriculumHeading(value: string | null): string {
  ...
  return value.replace(/\s+/g, ' ').replace(/\bSem\b/i, 'Semester').trim()
}

function formatCurriculumSubtitle({ coreHeading, extraHeading }: { coreHeading: string | null; extraHeading: string | null }): string | null {
  const sanitizedExtra = extraHeading?.trim() ?? null
  if (sanitizedExtra && !shouldSuppressSubtitle(sanitizedExtra)) return sanitizedExtra
...
function parseEligibility({ eligibility }: { eligibility: string | null }): ParsedEligibility {
...
function buildProgramLogistics({ program }: { program: any }): ProgramLogisticItem[] {
  const items: ProgramLogisticItem[] = []

  if (program.deliveryMode) items.push({ label: 'Mode of Study', value: formatDeliveryMode(program.deliveryMode) })
  if (program.durationMonths) items.push({ label: 'Duration', value: `${program.durationMonths} months` })
  if (program.degreeType) items.push({ label: 'Degree Path', value: program.degreeType })
  if (program.workExperienceMinYears) items.push({ label: 'Recommended Experience', value: `${program.workExperienceMinYears}+ years` })
  if (program.applicationDeadline) items.push({ label: 'Application Deadline', value: formatDate(program.applicationDeadline) })

  return items
}

function buildFinancialSummary({ program }: { program: any }): FinancialSummaryItem[] {
  const items: FinancialSummaryItem[] = []

  if (typeof program.totalFee === 'number' && program.totalFee > 0) {
    items.push({ label: 'Total Program Fee', value: formatIndianCurrency(program.totalFee) })
  }

  if (program.emiAvailable && typeof program.totalFee === 'number' && program.totalFee > 0) {
    items.push({
  ...
  if (typeof program.applicationFee === 'number' && program.applicationFee > 0) {
    items.push({ label: 'Application Fee', value: formatIndianCurrency(program.applicationFee) })
  }
...
function parseFeeBreakdown({ fees }: { fees: any[] }): FeeBreakdownRow[] {
  ...
  return fees.reduce<FeeBreakdownRow[]>((acc, fee, index) => {
    ...
    if (!rawLabel && (amountNumber === null || Number.isNaN(amountNumber))) return acc

    const match = rawLabel.match(/^(?<title>[^()]+?)(?:\((?<note>[^)]+)\))?$/)
    const label = (match?.groups?.title ?? rawLabel ?? `Component ${index + 1}`).trim() || `Component ${index + 1}`
    const note = match?.groups?.note ? match.groups.note.trim() : null
    const amount = amountNumber !== null && !Number.isNaN(amountNumber) ? formatIndianCurrency(amountNumber) : '—'
