...
export default function ProgramCatalog({
  ...
  const UNI_IMAGE_MAP: Record<string, { image?: string; logo?: string }> = {
    ...
    chandigarh: {
      image: "/uni/chandigarh.webp",
      logo: "/logos/chandighar.jpeg",
    },
  };
  ...
  function findAssets(
    ...
    if (institutionName)
      candidates.push(institutionName.toLowerCase().replace(/\s+/g, ""));
  ...
  useEffect(() => {
    try {
      ...
      if (!inst) {
        ...
        setFilters((prev) =>
          prev.institution ? { ...prev, institution: null } : prev
        );
        return;
      }
  ...
  const filteredPrograms = useMemo(() => {
    return programs.filter((program) => {
      ...
      const matchesDegree =
        ...
        program.degreeType === filters.degreeType;
      const matchesMode =
        filters.deliveryMode === "all" ||
        program.deliveryMode === filters.deliveryMode;
    ...
    return (
      <div className="bg-white border-2 border-slate-200 rounded-2xl p-12 text-center">
        ...
        <p className="text-slate-600 mb-6">
          Our team is adding more courses right now. Share your goals and we’ll
          notify you as soon as matches are live.
        </p>
        <a
          href="/lead-form"
          className="inline-block bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-full font-semibold"
        >
    ...
    <div className="space-y-8">
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          <div>
            ...
            <select
              value={filters.emi}
              onChange={(event) =>
                setFilters((prev) => ({ ...prev, emi: event.target.value }))
              }
              className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-[var(--brand)] focus:outline-none"
            >
              <option value="all">All</option>
      ...
      {filteredPrograms.length === 0 ? (
        <div className="bg-white border-2 border-slate-200 rounded-2xl p-12 text-center">
          ...
          <p className="text-slate-600 mb-6">
            Try adjusting your filters or tell us about your goals for tailored
            suggestions.
          </p>
          <a
            href="/lead-form"
            className="inline-block bg-[var(--brand)] hover:opacity-90 text-white px-6 py-3 rounded-full font-semibold"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredPrograms.map((program) => {
            ...
            const maybeInstitutionLogo =
              (program as any).institutionLogo ??
              (program as any).logoUrl ??
              (program as any).logo ??
              undefined;
            const maybeHeroImage =
              (program as any).heroImage ??
              (program as any).imageUrl ??
              undefined;

            const imageSrc =
              getCloudinaryFetchUrl(
                maybeHeroImage ?? assets.image ?? undefined
              ) ??
              maybeHeroImage ??
              assets.image ??
              undefined;
            ...
            return (
              <article
                key={program.id}
                className="group relative flex h-full flex-col justify-between rounded-3xl border border-slate-100 bg-white p-3 md:p-8 shadow-sm overflow-hidden transition-all hover:shadow-xl hover:border-[var(--brand)] hover:scale-[1.02] min-h-[14rem]"
              >
                {/* top content: buttons always visible so no conditional padding needed */}
                <div className="flex flex-col justify-between flex-1">
                  <div className="flex-1">
                            ...
                            <div className="mt-1 flex flex-wrap gap-2">
                              {program.institutionAccreditation
                                ...
                                .map((badge, idx) => (
                                  <span
                                    key={idx}
                                    className="inline-block bg-slate-100 text-slate-700 text-[10px] font-medium px-2 py-0.5 rounded-full"
                                  >
                                    {badge}
                                  </span>
                                ))}
                    ...
                    <div className="mb-2 min-h-[40px]">
                      <h2 className="text-base md:text-2xl font-bold text-slate-900 line-clamp-3">
                        {program.title}
                      </h2>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3 p-2 bg-slate-50 rounded-xl">
                      <div>
                        ...
                        <div className="text-xs md:text-sm font-bold text-slate-900">
                          {formatDeliveryModeShort(program.deliveryMode) ?? "-"}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">
                        ...
                        <div className="text-xs md:text-sm font-bold text-slate-900">
                          {formatDurationShort(program.durationMonths) ?? "-"}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">
                        ...
                        <div className="text-xs md:text-sm font-bold text-slate-900">
                          {formatFee(program.totalFee) ?? "TBD"}
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Action buttons - always visible, responsive text that shrinks but never wraps */}
                  <div className="pt-3 border-t border-slate-200 mt-auto">
                    <div className="flex items-center gap-2 sm:gap-3 flex-nowrap">
                      <a
                        href={`/programs/${program.id}`}
                        className="inline-flex items-center justify-center bg-[var(--brand)] text-white px-2 sm:px-3 md:px-4 py-2 rounded-full font-semibold text-[10px] sm:text-xs md:text-sm whitespace-nowrap flex-shrink-0"
                      >
                        View
                      </a>
                      <Link
                        ...
                        className="inline-flex items-center justify-center bg-white text-[var(--brand)] border border-[var(--brand)] px-2 sm:px-3 md:px-4 py-2 rounded-full font-semibold text-[10px] sm:text-xs md:text-sm whitespace-nowrap flex-shrink-0"
                      >
                        Apply Now
                      </Link>
                      <a
                        href={`/programs/${program.id}#curriculum`}
                        className="text-[10px] sm:text-xs md:text-sm font-semibold text-[var(--brand)] hover:underline whitespace-nowrap flex-shrink-0 ml-auto"
                      >
                        Curriculum
                      </a>
                    </div>
